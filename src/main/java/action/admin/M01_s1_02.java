...
public class M01_s1_02 implements Action {
	...
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if (birth_[0] == null || "".equals(birth_[0]) || birth_[1] == null || "".equals(birth_[1]) || birth_[2] == null
				|| "".equals(birth_[2])) {
			forward.setErrorMsg("生年月日を入力してください。");
			return forward;
		}
		if (birth_[0].length() < 4 || birth_[1].length() < 2 || birth_[2].length() < 2 || !birth_[0].matches(patternNum)
				|| !birth_[1].matches(patternNum) || !birth_[2].matches(patternNum)) {
		...
		int h_num = adminDAO.findByH_nameToH_num(hospital_name);
		if (h_num == 0) {
			forward.setErrorMsg("該当の病院は登録されていません。");
			return forward;
		}
		if(d_num_ == null || "".equals(d_num_)) {
			forward.setErrorMsg("該当の医者番号がみつかりませんでした。");
			return forward;
		}
		...
		if (isModifyPassword && isModifyEmail) {
			...
			if (!adminDAO.updateDoctorWithEmailAndPassword(doctor)) {
				forward.setErrorMsg("変更が失敗しました。");
				return forward;
			}
		} else {
			if (!adminDAO.updateDoctor(doctor)) {
				forward.setErrorMsg("変更が失敗しました。");
				return forward;
			}
		}
