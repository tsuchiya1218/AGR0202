...
public class U07_02 implements Action {
	...
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		String[] check_datetype_brith = xssFilter.stripTagAll(request.getParameterValues("brith"));
		...
		System.out.println("u07_02 action childBean check");
		System.out.println(map.get("name") + map.get("kana"));
		...
		try {
			if (map.get("brith").length() != 10 || Integer.parseInt(check_datetype_brith[1]) < 1
					|| Integer.parseInt(check_datetype_brith[1]) > 12) {
		...
		childBean.setC_brith(map.get("brith"));
