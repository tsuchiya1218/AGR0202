function isSubmit(form){
    ...
    let isError = false;
    let items_req = document.querySelectorAll('.items_req input');
    let regExp = /\d$/;
    let regSpace = /\s/;
    
    //空白検査 + 病名,使用期間
    for(let j of items_req){
        if(j.value === "" || j.value === null || regSpace.test(j.value)) {
            regExceptionStr += j.previousElementSibling.textContent + " ";
            isError = true;
        }
    }
    regExceptionStr += "\n";
    ...
    for(let i of items_medicine){
        if(count % 7 == 0){
             regExceptionStr += i.value + "\n";
        }
        if(i.getAttribute('type') === 'number' && !regExp.test(i.value) || regSpace.test(i.value)){
            isError = true;
        }
        count++;
    }
